...
const formatResults = (status, change) => {
   showChangeDue.innerHTML = `<p>Status: ${status}</p>`;
   change.map(money => (showChangeDue.innerHTML += `<p>${money[0]}: $${money[1]}</p>`));
   return;
};
const validateCashRegister = () => {
    ...
    if(totalCID < changeDue){
        return (showChangeDue.innerHTML = '<p>Status: INSUFFICIENT_FUNDS</p>');
    }
